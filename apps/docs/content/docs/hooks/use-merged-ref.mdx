...
`use-merged-ref` accepts any amount of refs and returns a function that should be passed to dom node. Use this hook when you need to use more than one ref on single dom node, for example, when you want to use [use-click-outside](/docs/hooks/use-click-outside) and [use-focus-trap](/docs/hooks/use-focus-trap) hooks and also get a ref for yourself:
...
```tsx
import { useRef } from "react";
import { useClickOutside, mergeRefs } from "@bigcomponents/hooks";
...
`mergeRefs` works the same way as `useMergedRef` but does not use hooks internally. Use it only when you cannot use `useMergedRef`. Note that `mergeRefs` will not work correctly with [use-focus-trap](/docs/hooks/use-focus-trap) hook, you are required to use `useMergedRef` with it.
