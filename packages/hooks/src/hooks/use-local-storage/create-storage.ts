...
function serializeJSON<T>(value: T, hookName: string) {
  try {
    ...
    throw new Error(
      `@bigcomponents/hooks ${hookName}: Failed to serialize the value`,
    );
  }
}
...
export function createStorage<T>(type: StorageType, hookName: string) {
  const eventName =
    type === "localStorage"
      ? "bigcomponents-local-storage"
      : "bigcomponents-session-storage";
  ...
  return function useStorage({
    ...
    useWindowEvent(eventName as any, (event) => {
